<div x-data="{ open: {{ isset($open) && $open ? 'true' : 'false' }}, working: false }" x-cloak>
    ...
    <div x-show="open"
        class="fixed z-50 mt-96 bottom-0 inset-x-0 px-4 pb-4 sm:inset-0 h-full sm:flex sm:items-center sm:justify-center">
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="ease-in duration-200"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
            class="fixed inset-x-0 h-full transition-opacity">
            <div class="absolute h-full inset-x-0 bg-gray-500 opacity-75"></div>
        </div>
