<div>
    ...
    <div class="relative">
            ...
            <div class="flex-grow h-10 flex items-center">
                ...
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="w-full pl-10 py-3 text-sm leading-4 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Search')}}" type="text" />
        ...
        <div class="mt-3 rounded-lg shadow-lg bg-white max-w-screen overflow-x-scroll">
                ...
                <div class="table align-middle min-w-full">
                    ...
                    <div class="table-row divide-x divide-gray-200">
                        @foreach($this->columns as $index => $column)
                            ...
                            <div class="w-32 py-4 flex justify-center overflow-hidden align-top px-6 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-black uppercase tracking-wider focus:outline-none">
                    ...
                    @forelse($this->results as $result)
                        <div class="table-row p-1 divide-x divide-gray-100 {{ isset($result->checkbox_attribute) && in_array($result->checkbox_attribute, $selected) ? 'bg-orange-100' : ($loop->even ? 'bg-gray-100' : 'bg-gray-50') }}">
                            @foreach($this->columns as $column)
                                ...
                                @else
                                    <div class="px-6 py-2 whitespace-no-wrap text-sm leading-5 text-black table-cell @if($column['align'] === 'right') text-right @elseif($column['align'] === 'center') text-center @else text-left @endif">
                                        {!! $result->{$column['name']} !!}
                                    </div>
                                @endif
                            @endforeach
